...
const event: Event<Events.MessageCreate> = {
  ...
  async execute(message) {
    ...
    const myMention = new RegExp(
      `<@((!?${message.client.user!.id}${botRole ? `)|(&${botRole.id}` : ""}))>`,
      "g",
    );
    ...
    try {
      if (modelInfo == null) {
        let rawResponse: string | OllamaShowResponse =
          await makeRequest<string>("/api/show", "post", {
            name: config.model,
          });
        if (typeof rawResponse === "string")
          rawResponse = JSON.parse(rawResponse) as OllamaShowResponse;
      ...
      const channelName =
        "name" in message.channel ? message.channel.name : "DMs";
      log.log(
        LogLevel.Debug,
        `${message.guild ? `#${channelName}` : "DMs"} - ${message.author.username}: ${userInput}`,
      );
      ...
      try {
        ...
        response = parseJSONLines<OllamaGenerateChunk>(
          rawResponse,
          "Ollama stream",
        );
      } catch (error) {
      ...
      const prefix: string =
        config.showStartOfConversation && messages[channelID].amount == 0
      ...
      if (doneChunk?.context) {
        ...
        log.log(
          LogLevel.Error,
          "Ollama response missing final context; conversation state not updated",
        );
      }
    } catch (error) {
      if (typing) {
